...
import novitiusTexture from "/images/neptune.jpg";
...
function identifyPlanet(clickedObject) {
  ...
  if (clickedObject.material === mercury.planet.material) {
    ...
    offset = 25;
    return uranus;
  } else if (clickedObject.material === novitius.planet.material) {
    offset = 20;
    return novitius;
...
const novitius = new createPlanet("Novitius", 24 / 4, 340, 28, novitiusTexture);
...
const planetData = {
  ...
  Novitius: {
    ...
    info: "Novitius. Planeta, gdzie mozesz rozpoczac swoja przygode z Magia SZachowa. Znajdziesz tu wiele tajemnic i nieodkrytych zakatkow, ktore czekaja na odkrycie.",
  },
  Pluto: {
    radius: "1,188.3 km",
    tilt: "122.53°",
    ...
    info: "Novitius. Planeta, gdzie możesz rozpocząć swoją przygodę z Magią SZachową. Znajdziesz tu wiele tajemnic i nieodkrytych zakątków, które czekają na odkrycie.",
  },
};
...
novitius.planet.receiveShadow = true;
...
function animate() {
  ...
  novitius.planet.rotateY(0.005 * settings.acceleration);
  novitius.planet3d.rotateY(0.00008 * settings.accelerationOrbit);
  ...
  if (intersects.length > 0) {
    ...
    if (intersectedObject === earth.Atmosphere) {
      ...
      // --- Dodane: wyświetlenie napisu "Peritus" w miejscu podświetlonego obiektu ---
      if (!window._earthLabel) {
        const label = document.createElement("div");
        label.innerText = "Peritus";
        ...
        window._earthLabel = label;
      }
      const earthLabel = window._earthLabel;
      // Oblicz pozycję 2D na ekranie na podstawie pozycji 3D Ziemi
      const earthPos = earth.planet.getWorldPosition(new THREE.Vector3());
      earthPos.project(camera);
      const x = (earthPos.x * 0.5 + 0.5) * window.innerWidth;
      const y = (-earthPos.y * 0.5 + 0.5) * window.innerHeight;
      earthLabel.style.left = `${x}px`;
      earthLabel.style.top = `${y}px`;
      earthLabel.style.display = "block";
      // Ukryj label jeśli nie podświetlona
    } else if (intersectedObject === venus.Atmosphere) {
      outlinePass.selectedObjects = [venus.planet];
      // --- Dodane: wyświetlenie napisu "Expertus" w miejscu podświetlonego obiektu ---
      if (!window._venusLabel) {
        const label = document.createElement("div");
        label.innerText = "Expertus";
        label.style.position = "fixed";
        label.style.pointerEvents = "none";
        label.style.background = "rgba(0,0,0,0.7)";
        label.style.color = "#fff";
        ...
        label.style.display = "none";
        document.body.appendChild(label);
        window._venusLabel = label;
      }
      const venusLabel = window._venusLabel;
      // Oblicz pozycję 2D na ekranie na podstawie pozycji 3D Wenus
      const venusPos = venus.planet.getWorldPosition(new THREE.Vector3());
      venusPos.project(camera);
      const x = (venusPos.x * 0.5 + 0.5) * window.innerWidth;
      const y = (-venusPos.y * 0.5 + 0.5) * window.innerHeight;
      venusLabel.style.left = `${x}px`;
      venusLabel.style.top = `${y}px`;
      venusLabel.style.display = "block";
      // Ukryj label jeśli nie podświetlona
    } else if (intersectedObject === mercury.planet) {
      outlinePass.selectedObjects = [mercury.planet];
      if (!window._mercuryLabel) {
        const label = document.createElement("div");
        label.innerText = "Magister";
        label.style.position = "fixed";
        label.style.pointerEvents = "none";
        label.style.background = "rgba(0,0,0,0.7)";
        label.style.color = "#fff";
        ...
        label.style.display = "none";
        document.body.appendChild(label);
        window._mercuryLabel = label;
      }
      const mercuryLabel = window._mercuryLabel;
      // Oblicz pozycję 2D na ekranie na podstawie pozycji 3D Merkurego
      const mercuryPos = mercury.planet.getWorldPosition(new THREE.Vector3());
      mercuryPos.project(camera);
      const x = (mercuryPos.x * 0.5 + 0.5) * window.innerWidth;
      const y = (-mercuryPos.y * 0.5 + 0.5) * window.innerHeight;
      mercuryLabel.style.left = `${x}px`;
      mercuryLabel.style.top = `${y}px`;
      mercuryLabel.style.display = "block";
      // Ukryj label jeśli nie podświetlona
    } else {
    ...
    // Ukryj label jeśli nie podświetlona Ziemia
    if (window._earthLabel) window._earthLabel.style.display = "none";
    // Ukryj label jeśli nie podświetlona Venus
    if (window._venusLabel) window._venusLabel.style.display = "none";
    // Ukryj label jeśli nie podświetlona Mercury
    if (window._mercuryLabel) window._mercuryLabel.style.display = "none";
  }
  // ******  ZOOM IN/OUT  ******
